...
# Load results from the RF and LR CSV files
rf_results_df = pd.read_csv('rf_results.csv')
lr_results_df = pd.read_csv('lr_results.csv')

# Define the weights for RF and LR models
rf_weight = 0.5  # Weight for RF predictions
lr_weight = 0.5  # Weight for LR predictions
...
for param in parameters:
    for horizon in prediction_horizons:
        # Get RF and LR results for the current parameter and horizon
        rf_row = rf_results_df[(rf_results_df['Parameter'] == param) & (rf_results_df['Prediction Horizon (n)'] == horizon)].iloc[0]
        lr_row = lr_results_df[(lr_results_df['Parameter'] == param) & (lr_results_df['Prediction Horizon (n)'] == horizon)].iloc[0]

        # Calculate the weighted average for each evaluation metric
        weighted_avg_results = {}
        for metric in ['MAE', 'MSE', 'RMSE', 'MAPE']:
            weighted_avg = rf_weight * rf_row[metric] + lr_weight * lr_row[metric]
...
average_results_df.to_csv('rf_lr.csv', index=False)
print("Average RF-LR Results saved to rf_lr.csv")
