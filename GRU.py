...
# Create a dictionary to store the GRU results
gru_results = {}
...
with open(csv_filename, 'w', newline='') as csv_file:
    ...
    for param in targets:
        for n in prediction_horizons:
            ...
            sequence_length = 20  # You can adjust this sequence length as needed
            X_train_sequences = []
            y_train_sequences = []
            ...
            # Reshape X_train_sequences to match GRU input shape
            ...
            # Align the lengths of y_test and gru_predictions
            y_test = y_test.iloc[sequence_length:-n].values
            gru_predictions = gru_predictions[:len(y_test)]  # Ensure the same length

            # Remove rows with NaN values from both y_test and gru_predictions
            nan_indices = np.isnan(y_test) | np.isnan(gru_predictions
            ...
            # Store the results in the gru_results dictionary
